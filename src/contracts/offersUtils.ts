...
import { OfferAccepted as OfferAcceptedEvent } from "../../generated/Offers/Offers";
...
export function updateOffers(event: OfferAcceptedEvent): void {
  ...
  if (tx) {
    ...
    if (!saleEntity && tx.unmatchedTransferCount > 0) {
      ...
      if (currencyEntity) {
        ...
        saleEntity.strategy = "fixedSale";
        saleEntity.amount = event.params.value.divDecimal(
